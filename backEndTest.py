...
class TestMemeCacheModule(TestCase):
    ...
    def setUp(self):
        # cache path settings
        memeCache.cache_folder = 'test_cache'  # cache folder name. Default = cache
        memeCache.cache_file_name = 'cache.pickle'  # Default = cache.pickle
        memeCache.cache_file_path = os.path.join(memeCache.cache_folder, memeCache.cache_file_name)

        # MemeBox path settings
        memeCache.memebox_folder = 'test_memebox'
        memeCache.memebox_file = 'memebox.pickle'
        memeCache.memebox_file_path = os.path.join(memeCache.memebox_folder, memeCache.memebox_file)

    def tearDown(self):
        # remove temp folder & file
        if os.path.isdir(memeCache.cache_folder):
            shutil.rmtree(memeCache.cache_folder)
        if os.path.isdir(memeCache.memebox_folder):
    ...
    def test_save_load_memebox(self):
        ...
        # It should have created folder & file
        self.assertTrue(os.path.isdir(memeCache.memebox_folder))  # verify that folder exists
        self.assertTrue(os.path.exists(memeCache.memebox_file_path))  # verify that file exists
...
class TestMemeFinderModule(TestCase):
    ...
    def test_find_meme_api_call(self):
        ...
        memes = memeFinder.find_meme('cat', True)
    ...
    @patch('memeFinderReddit.create_meme_object')
    @patch('memeFinderImgur.create_meme_object')
    @patch('memeFinderGiphy.create_meme_object')
    @patch('memeFinder.get_fresh_memes')
    def test_find_meme_cache(self, mock_get_fresh_memes, mock_giphy_create_object, mock_imgur_create_object, mock_reddit_create_object) :
        ...
        memeCache.pickle_data(temp_cache)
        memeCache.unpickle_data()
    ...
    def test_pick_meme_create_object(self):
        ...
        # execute pick_meme
        giphy_meme = memeFinder.pick_meme(giphy_memecache)
        imgur_meme = memeFinder.pick_meme(imgur_memecache)
        reddit_meme = memeFinder.pick_meme(reddit_memecache)
